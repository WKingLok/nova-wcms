<template>
    <LoadingView :loading="initialLoading">
        ...
        <div
            :class="{
                'mt-6':
                    shouldShowCards && hasDetailOnlyCards && cards.length > 0,
            }"
            ...
            >
                <div v-if="panel.showToolbar" class="md:flex items-center mb-3">
                    ...
                    <div class="ml-auto flex items-center">
                        ...
                        <!-- Approval Archive Link -->
                        <Link
                            v-if="resource.approval"
                            v-tooltip="{
                                placement: 'bottom',
                                distance: 10,
                                skidding: 0,
                            ...
                            :href="
                                $url(
                                    `/resources/${resourceName}/${resourceId}/archive`,
                                )
                            "
        ...
        <div
            class="al-flex al-flex-col al-justify-center al-space-y-2 md:al-flex-row md:al-items-center md:al-justify-end md:al-space-x-3 md:al-space-y-0"
            v-if="resource && resource.approval"
        >
            <NovaUiButton
                v-for="(action, index) in resource.approval.actions"
                :key="index"
                @click="approvalAction(action)"
                :label="action.label"
            />
        </div>
    </LoadingView>
...
export default {
    ...
    methods: {
         ...
         */
        async approvalAction(action) {
            try {
                const {
                    data: { redirect },
                } = await Nova.request().post("/nova-api/approval/actions", {
                    resourceName: this.resourceName,
                    resourceId: this.resourceId,
